...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderSalesmen() {
    ...
    salesmen.forEach(s => {
      ...
      row.innerHTML = `
        <td>${s.name}</td>
        <td><button class="btn btn-sm btn-danger" onclick="deleteSalesman(${s.id})">🗑️</button></td>
      `;
  ...
  window.deleteSalesman = function (id) {
    if (!confirm(t('delete_employee_confirm'))) return;
  ...
  function renderMonthlyTargets() {
    ...
    targets.forEach(t => {
      ...
      row.innerHTML = `
        ...
        <td>${t.month}</td>
        <td>${t.year}</td>
        <td>${t.target.toFixed(2)} ${t('EGP', 'ج.م')}</td>
      `;
  ...
  function renderPerformance() {
    ...
    targets.forEach(t => {
      ...
      row.innerHTML = `
        ...
        <td>${t.year}</td>
        <td>${t.target.toFixed(2)} ${t('EGP', 'ج.م')}</td>
        <td>${achieved.toFixed(2)} ${t('EGP', 'ج.م')}</td>
        <td>${percent} %</td>
      `;
